...
mcp = FastMCP(
    ...
    instructions="MEOK AI Labs — A2A Governance Bridge. Compliance checking for agent-to-agent transactions. First in market.",
)
...
def verify_agent_compliance(
    ...
    for fw in frameworks:
        if fw == "eu_ai_act":
            checks[fw] = {
                "has_risk_assessment": any(w in desc for w in ["risk", "assessment"]),
            ...
            checks[fw] = {
                "has_data_governance": any(
                    w in desc for w in ["data", "privacy", "gdpr"]
                ),
                "has_consent_mechanism": any(
                    w in desc for w in ["consent", "permission"]
                ),
            }
        else:
    ...
    trust_score = sum(sum(v.values()) for v in checks.values()) / max(
        sum(len(v) for v in checks.values()), 1
    )

    _trust_registry[agent_id] = {
    ...
    return {
        ...
        "details": checks,
        "recommendation": "Agent cleared for A2A transactions"
        if all_pass
        else "Agent fails compliance — block transaction",
    }


@mcp.tool()
def authorize_a2a_transaction(
    source_agent: str,
    target_agent: str,
    ...
    txn_id = hashlib.sha256(
        f"{source_agent}{target_agent}{datetime.now(timezone.utc).isoformat()}".encode()
    ).hexdigest()[:12]
    _audit_trail.append(
        {
            "type": "transaction",
    ...
    return {
        ...
        "governance_note": "Both agents verified. Transaction authorized."
        if authorized
        else "One or both agents not verified. Run verify_agent_compliance first.",
    }
...
def cross_agent_risk_score(
    ...
    complexity_mult = {"low": 0.8, "medium": 1.0, "high": 1.3, "critical": 1.5}.get(
        task_complexity, 1.0
    )
    risk = round(max(0, 1.0 - avg) * complexity_mult, 2)
    return {
        "agents": agent_list,
        "individual_trust": dict(zip(agent_list, scores)),
        "average_trust": round(avg, 2),
        "task_complexity": task_complexity,
        "composite_risk": risk,
        "recommendation": "Proceed"
        if risk < 0.5
        else "Add human oversight"
        if risk < 0.8
