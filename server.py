...
@mcp.tool()
def verify_agent_compliance(agent_id: str, agent_description: str, required_frameworks: str = "eu_ai_act") -> str:
    ...
    return json.dumps({"agent_id": agent_id, "compliant": all_pass, "trust_score": round(trust_score, 2),
        "frameworks_checked": frameworks, "details": checks,
        "recommendation": "Agent cleared for A2A transactions" if all_pass else "Agent fails compliance — block transaction"}, indent=2)

@mcp.tool()
def authorize_a2a_transaction(source_agent: str, target_agent: str, task_description: str, data_types: str = "") -> str:
    ...
    return json.dumps({"transaction_id": txn_id, "authorized": authorized,
        ...
        "governance_note": "Both agents verified. Transaction authorized." if authorized else "One or both agents not verified. Run verify_agent_compliance first."}, indent=2)

@mcp.tool()
def get_trust_registry() -> str:
    """Get the current agent trust registry."""
    return json.dumps({"agents": _trust_registry, "total": len(_trust_registry)}, indent=2)

@mcp.tool()
def get_a2a_audit_trail(limit: int = 20) -> str:
    """Get audit trail of all A2A governance checks."""
    return json.dumps({"total": len(_audit_trail), "recent": _audit_trail[-limit:]}, indent=2)

@mcp.tool()
def cross_agent_risk_score(agents: str, task_complexity: str = "medium") -> str:
    ...
    return json.dumps({"agents": agent_list, "individual_trust": dict(zip(agent_list, scores)),
        "average_trust": round(avg, 2), "task_complexity": task_complexity,
        "composite_risk": risk, "recommendation": "Proceed" if risk < 0.5 else "Add human oversight" if risk < 0.8 else "Block transaction"}, indent=2)
